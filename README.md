...
If your bundle includes pre-built image tarballs (`images/` directory):

```bash
docker load -i images/deepsql-backend.tar
docker load -i images/deepsql-frontend.tar
```

Then set this in your `.env` (step 2) to skip pulling from the registry:

```bash
DEEPSQL_SKIP_IMAGE_PULL=true
```

If DeepSQL has provided GHCR registry access instead, log in first:
...
| `DEEPSQL_BACKEND_IMAGE` | Backend Docker image reference (pre-filled) |
| `DEEPSQL_FRONTEND_IMAGE` | Frontend Docker image reference (pre-filled) |

Security secrets (`SECURITY_JWT_SECRET`, `ENCRYPTION_KEY`, `DB_PASSWORD`) are **auto-generated** by the install script if left as placeholders. You do not need to set them manually.
...
- Pull or verify the DeepSQL Docker images
...
Then restart: `./scripts/install.sh` or `docker compose --project-name deepsql-selfhost up -d`.
...
When DeepSQL provides a new release:

**If you received new image tarballs:**
...
Update `.env` with the new image tags, then:

```bash
./scripts/install.sh
```

**If you have GHCR registry access:**

Update `.env` to the new version:
...
If you have image tarballs, load them with `docker load` and set `DEEPSQL_SKIP_IMAGE_PULL=true` in `.env`.

If you have GHCR access, log in first:

```bash
echo '<YOUR_TOKEN>' | docker login ghcr.io -u <YOUR_USERNAME> --password-stdin
```

Contact DeepSQL support if you do not have a token or image tarballs.
